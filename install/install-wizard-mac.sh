...
echo "Enter the absolute path to the genesis.txn file (the default assumes you are using Vagrant, and will use /vagrant/config/genesis.txn"
read -p ':' genesispath
echo $genesispath
...
if [ -z "$genesispath" ]
then
      echo "\$genesispath is empty, using default value of /vagrant/config/genesis.txn"
...
sudo apt-get install -y libnullpay=1.12.0 &&
...
if [  -d "/home/vagrant/.indy_client/wallet" ]; then
